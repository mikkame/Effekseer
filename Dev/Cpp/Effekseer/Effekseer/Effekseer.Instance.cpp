...
private:
	constexpr double getFade(const double t_) const noexcept { return t_ * t_ * t_ * (t_ * (t_ * 6 - 15) + 10); }
	constexpr double getLerp(const double t_, const double a_, const double b_) const noexcept { return a_ + t_ * (b_ - a_); }
	...
	double setNoise(double x_ = 0.0, double y_ = 0.0, double z_ = 0.0) const noexcept
	{
		...
		return this->getLerp(
			...
			this->getLerp(
				...
				this->getLerp(
					u, this->getGrad(this->p[a2 + 1], x_, y_ - 1, z_ - 1), this->getGrad(this->p[b2 + 1], x_ - 1, y_ - 1, z_ - 1))));
	}

	template <typename... Args> double noise(const Args... args_) const noexcept { return this->setNoise(args_...) * 0.5 + 0.5; }
...
public:
	template <typename... Args> double octaveNoise(const std::size_t octaves_, const Args... args_) const noexcept
	{
		return this->setOctaveNoise(octaves_, args_...) * 0.5 + 0.5;
	}
};

PerlinNoise pnoise1(1);
PerlinNoise pnoise2(277);
PerlinNoise pnoise3(3777);

Vector3D CurlNoise(Vector3D pos)
{
	pos *= 0.2f;
	...
	const Vector3D dx = Vector3D(e, 0.0, 0.0);
	const Vector3D dy = Vector3D(0.0, e, 0.0);
	const Vector3D dz = Vector3D(0.0, 0.0, e);

	auto noise = [](Vector3D v) -> Vector3D {
		return Vector3D(
			pnoise1.octaveNoise(2, v.X, v.Y, v.Z), pnoise2.octaveNoise(2, v.X, v.Y, v.Z), pnoise3.octaveNoise(2, v.X, v.Y, v.Z));
	};

	Vector3D p_x0 = noise(pos - dx);
	Vector3D p_x1 = noise(pos + dx);
	Vector3D p_y0 = noise(pos - dy);
	Vector3D p_y1 = noise(pos + dy);
	Vector3D p_z0 = noise(pos - dz);
	Vector3D p_z1 = noise(pos + dz);

	float x = (p_y1.Z - p_y0.Z) - (p_z1.Y - p_z0.Y);
	float y = (p_z1.X - p_z0.X) - (p_x1.Z - p_x0.Z);
	float z = (p_x1.Y - p_x0.Y) - (p_y1.X - p_y0.X);

	return Vector3D(x, y, z) * invE2 * 0.5;
}
...
void Instance::CalculateMatrix( float deltaFrame )
{
	...
	if( m_pEffectNode->GetType() != EFFECT_NODE_TYPE_ROOT )
	{
		...
		noiseRevision_ += CurlNoise(localPosition + noiseRevision_) * 0.1f;

		localPosition += noiseRevision_;
